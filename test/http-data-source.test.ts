...
test('Should respond with stale-if-error cache on origin error if maxTtlIfError was not set on global level cache configuration, but was overridden by method level configuration', async (t) => {
  t.plan(12)
...
test('Should not respond with stale-if-error cache on origin error if maxTtlIfError was set on global level cache configuration, but was not set in method level configuration', async (t) => {
  t.plan(8)
